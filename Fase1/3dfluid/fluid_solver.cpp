...
void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
    ...
    for (int l = 0; l < LINEARSOLVERTIMES; l++) {
        // Tiling loops
        for (int kk = 1; kk <= M; kk += BLOCKSIZE) {
            for (int jj = 1; jj <= N; jj += BLOCKSIZE) {
                for (int ii = 1; ii <= O; ii += BLOCKSIZE) {
                    
                    // Inner loops over the block
                    for (int k = kk; k < kk + BLOCKSIZE && k <= M; k++) {
                        for (int j = jj; j < jj + BLOCKSIZE && j <= N; j++) {
                            for (int i = ii; i < ii + BLOCKSIZE && i <= O; i+=2) {
                                int xCenter1 = IX(i, j, k);
                                int xCenter2 = IX(i + 1, j, k);

                                float auxA1 = x[xCenter2];
                                float auxA2 = x[IX(i, j - 1, k)] + x[IX(i, j + 1, k)];
                                float auxA3 = x[IX(i, j, k - 1)] + x[IX(i, j, k + 1)];
                                float sumA = auxA1 + auxA2 + auxA3;

                                float auxB1 =  x[IX(i + 2, j, k)];
                                float auxB2 = x[IX(i+1, j - 1, k)] + x[IX(i+1, j + 1, k)];
                                float auxB3 = x[IX(i+1, j, k - 1)] + x[IX(i+1, j, k + 1)];
                                float sumB = auxB1 + auxB2 + auxB3;

                                x[xCenter1] = (x0[xCenter1] * invC + auxA * (sumA+x[IX(i - 1, j, k)]));
                                x[xCenter2] = (x0[xCenter2] * invC + auxA * (sumB +x[xCenter1]));
                            }
                        }
                    }
                }
